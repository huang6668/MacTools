...
- `DuoStatusPro` displays battery, network, and volume snapshots in one standalone or host-primary menu-bar icon. Like Duo Status, its placement is requested through the host and it exposes only appearance settings, not a repeatable mutation. Its system battery and Wi-Fi hiding switches are persisted display preferences rather than repeatable actions.
